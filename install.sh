...
sudo apt-get update
sudo apt-get install -y apt-file agrep certbot cloc curl djinn fish git haskell-stack hlint hoogle ipython3 jq lambdabot nodejs npm pandoc pdfsam powerline pylint python-wxtools python3-pytest rename supervisor tox yarn zbar-tools xdotool
