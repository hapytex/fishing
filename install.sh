...
sudo apt-get update
sudo apt-get install -y apt-file agrep certbot cloc curl djinn fish git git-lfs haskell-stack hlint hoogle ipython3 jq lambdabot nodejs npm pandoc pdfsam powerline pylint python-wxtools python3-pytest rename supervisor tig tox yarn zbar-tools xdotool
