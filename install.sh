...
sudo apt-get update
sudo apt-get install -y alarm-clock-applet apt-file agrep certbot cloc curl djinn fish git git-filter-repo git-lfs haskell-stack hlint hoogle ipython3 jq lambdabot nodejs npm pandoc pdfsam powerline pylint python-wxtools python3-pytest rename sdate supervisor swi-prolog-core tig tox yarn zbar-tools xdotool
