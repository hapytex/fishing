function ledger --description 'book a financial transaction and see the balance' -a am -a to -a nm -a dt -a fm -a cy -a rf
  set ass (assets)
  set task 'balance'
  if set -q argv[1]
    if [ "$argv[1]" = 'budget' ]
      ...
      test -n "$tx" && set tx "\n  tax  $cy$tx"
      set tos (string split '/' "$to") 'vat' 'ship' 'discount' 'tip' 'tax'
      set tot ''
      set idx 1
      for am in $ams
        set tot "$tot\n  $tos[$idx]  $cy$am"
  ...
  /usr/bin/ledger -f $ass/ledger/ledger.dat "$task" --current '^[^:]+[:][^:]+[:]*[^:]*$'
end
